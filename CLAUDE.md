...
- `pesde run test` loads all 24 test specs and runs them via frktest's lune reporter.
- All specs wired into `scripts/RunTests.luau`: Buffer, Serde (12 types + Referent), Artifact, Format, Instance, Compat, Cli, SchemaEvolution, Patch.
...
3. ✅ **Instance layer** — `src/instance/` serializes/deserializes Roblox instances with default elision. Whole-instance dedup (`templateRefs`) was removed in favor of column Dictionary encoding (see Milestone 8); `Deserializer.luau` still honors `templateRefs` when present for backward-compat reads of v1/v2 files.
...
8. ✅ **Column Dictionary Encoding** — added a 5th column encoding (id `2`) that captures repeated-but-shuffled values RLE misses. Wired into the cost-picker (`src/instance/Serializer.luau`, which keys unique dictionary values by their actual codec-serialized bytes — not `tostring(v)`, which can lose precision for Vector3/CFrame/etc and silently collapse distinct values) and read/write (`src/format/Writer.luau`/`Reader.luau`, plus the version-2 reader `src/format/readers/V2.luau`). `bench/DedupVsDictionary.luau` measured dedup contributing 0 additional bytes of savings once Dictionary encoding was active (Dictionary's per-column dictionary already collapses a fully-identical block to a 1-entry dictionary — dedup's best case). Based on that result, whole-instance dedup (`templateRefs`) was later removed entirely (see Milestone 3 and the v2→v3 bump below); `Deserializer.luau` retains read support for `templateRefs` only to stay backward-compatible with v1/v2 files.
9. ✅ **Schema Evolution** — `src/format/Reader.luau` now dispatches on the format version field to frozen per-version readers (`src/format/readers/V1.luau`, `V2.luau`, `V3.luau`) and pipes the parsed result through `compat.migrate` — previously the compat registry existed but was never called, and `Reader.luau` hard-asserted `version == 1`. v1→v2 was a wire-format change (templateRefs raw varints → RLE), proven against a frozen v1 binary fixture (`tests/fixtures/format_v1_sample.bin`). v2→v3 removed `templateRefs` entirely (whole-instance dedup removal). `Writer.luau` only ever writes the current version (3).
...
Lattice is feature-complete for all six previously-planned specs (extended codec coverage, column dictionary encoding, schema evolution, CLI tooling, incremental/delta encoding) plus whole-instance dedup removal. Current focus:
...
2. **Parallel encoding (multi-process, Lune prototype)** — `docs/superpowers/specs/2026-07-05-parallel-encoding-design.md` designs a per-class-block parallel serialize/deserialize split across Lune child processes (`@lune/process`), bumping the format to schema v4 (length-prefixed class blocks). Being implemented now.

Suggested future work (not yet scoped as specs):

3. **Reconcile duplicate identity-matching logic**: `cli/Diff.luau`'s `matchInstances` and `src/patch/Identity.luau` were built independently in isolated worktrees and solve the same UniqueId-with-positional-fallback problem. Unify into one shared module.
4. **`cli diff`'s cross-class-match gap**: when positional fallback pairs two instances of different classes, `cli/Diff.luau` currently drops the pair silently instead of reporting a class-mismatch. Low severity, flagged in code review.
...
- **`scripts/BuildPackage.luau` needs a Rojo sourcemap, generated at the project root**: without one, darklua's `convert_require` (roblox target) guesses the DataModel shape mirrors the literal `@src` alias path, producing broken `script.Parent:FindFirstChild('src')...` chains (there's no "src" sibling of `Lattice` in `default.project.json` — Rojo maps `dist/lattice`'s contents directly onto `ReplicatedStorage.Lattice`). Fix: build runs three passes — raw-copy `src/` to `dist/lattice` (so Rojo's `$path` resolves to real files), `rojo sourcemap default.project.json -o sourcemap.json`, then re-run darklua over `dist/lattice` in place with `rojo_sourcemap` pointed at that file. **The sourcemap file must live at the project root, not nested under `dist/`**: darklua resolves each sourcemap entry's file path relative to the sourcemap file's own directory, while `rojo sourcemap` always writes paths relative to its invocation cwd (project root) regardless of where `-o` points — nesting it under `dist/` breaks every path match silently (every require comes out unconverted, one `WARN` per require, no build error). This is why `.gitignore` already had a bare `sourcemap.json` entry.
- **`src/instance/Serializer.luau` must not unconditionally `require("@lune/...")` at module top level**: this file is required by `init.luau` and therefore loads inside real Roblox too. Lune-only APIs (`@lune/process`, `@lune/task`) are only valid on the Lune-worker-process fallback path (`serializeParallel`'s `else` branch, reached only when `RobloxBackend.isActorParallelAvailable()` is false) and must be required lazily inside that branch — mirroring the existing `pcall(require, "@lune/process")` — never as a top-level `local`, or Studio throws immediately on module load before the environment-detection logic ever runs.
- **`src/robloxDatatypes.luau`**: shared guarded-require for Lune's roblox-datatype bindings (`Vector3`, `CFrame`, `Color3`, `Enum`, `Instance`, etc.). Every serde codec plus `instance/Deserializer.luau` and `patch/apply.luau` needs these constructors, but Lune doesn't expose them as globals (only real Roblox does) — they used to each do their own unconditional `local roblox = require("@lune/roblox")`, which crashed the moment the module loaded inside real Roblox Studio (no `@lune` alias there). This module centralizes the `pcall(require, "@lune/roblox")` in one place and falls back to the real Roblox globals when the require fails, so every consumer just does `local roblox = require("@src/robloxDatatypes")` unchanged. `src/artifacts/build/buildArtifact.luau` still requires `@lune/roblox` directly and is left as-is — it's Lune-only build tooling, never reached by `init.luau`'s runtime require graph.
